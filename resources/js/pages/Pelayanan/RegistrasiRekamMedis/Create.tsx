...
import { ArrowLeft, Save, User, Building, Stethoscope, Phone, Shield } from 'lucide-react';
...
export default function Create({ companies, plants, departments, employeeStatuses, guarantors, isSuperAdmin }: Props) {
    const [formData, setFormData] = useState({
        ...
        employee_status_id: '',
        department_id: '',
        guarantors: [] as GuarantorForm[],
    });

    // Dynamic dropdown data untuk super admin
    const [dynamicPlants, setDynamicPlants] = useState<Plant[]>(isSuperAdmin ? [] : plants);
    ...
    useEffect(() => {
        if (isSuperAdmin && formData.company_id) {
            ...
            // Reset plant dan dependent fields ketika company berubah
            setFormData(prev => ({
                ...prev,
                plant_id: '',
                department_id: '',
    ...
    }, [formData.company_id, isSuperAdmin]);

    // Effect untuk fetch dropdown data ketika plant berubah (untuk super admin)
    useEffect(() => {
        if (isSuperAdmin && formData.company_id && formData.plant_id) {
            fetchDropdownData(formData.company_id, formData.plant_id);
            
            // Reset dependent fields ketika plant berubah
            setFormData(prev => ({
                ...prev,
                department_id: '',
                employee_status_id: '',
    ...
    }, [formData.plant_id, isSuperAdmin]);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        router.post(route('pelayanan.registrasi-rekam-medis.store'), submitData);
    };
            ...
            <div className="space-y-8">
                ...
                <div className="flex items-center justify-between mt-4">
                    <div className="flex items-center space-x-4">
                        ...
                        <div>
                            <h1 className="text-2xl font-semibold text-gray-900">Tambah Pasien Baru</h1>
                            <p className="text-sm text-gray-600">Masukkan data pasien baru ke dalam sistem</p>
                        </div>
                    </div>
                </div>
                            ...
                            <CardContent className="space-y-4">
                                ...
                                <div>
                                    ...
                                    >
                                        ...
                                        <SelectContent>
                                            {(isSuperAdmin ? dynamicPlants : plants).map((plant) => (
