...
import { Eye, Stethoscope, XCircle, ChevronLeft, ChevronRight, Search, Printer } from 'lucide-react';
...
const breadcrumbs = [
  ...
  { title: 'Rawat Jalan', href: '' },
];

const STATUS = [
  { value: 'waiting', label: 'Belum Diperiksa', color: 'bg-blue-100 text-blue-700' },
  { value: 'in_progress', label: 'Sedang Diperiksa', color: 'bg-yellow-100 text-yellow-700' },
  { value: 'completed', label: 'Selesai', color: 'bg-green-100 text-green-700' },
  { value: 'cancelled', label: 'Batal', color: 'bg-red-100 text-red-700' },
];

interface OutpatientQueue {
  id: string;
  waktu: string;
  nik: string;
  nip: string;
...
export default function RawatJalan({ outpatientQueues, filters, stats }: Props) {
  ...
  useEffect(() => {
    if (!filters.tanggal && localFilters.tanggal === todayStr) {
      router.get(route('pelayanan.rawat-jalan.index'), { ...localFilters, tanggal: todayStr, timezone_offset: timezoneOffset }, { preserveState: true, replace: true });
    }
    // eslint-disable-next-line
  ...
  const handleFilterChange = (field: string, value: string) => {
    ...
    if (field === 'search') {
      ...
      (window as any).searchTimeout = setTimeout(() => {
        router.get(route('pelayanan.rawat-jalan.index'), backendFilters, {
          preserveState: true,
          replace: true,
        });
      }, 300);
  ...
  const handleStatusUpdate = (queueId: string, newStatus: string, redirectTo?: string) => {
    router.patch(route('pelayanan.rawat-jalan.update-status', queueId), {
      status: newStatus
    }, {
      onSuccess: () => {
        if (redirectTo) {
    ...
    <AppLayout breadcrumbs={breadcrumbs}>
      <div className="mt-6 space-y-6">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-semibold text-gray-900">Rawat Jalan</h1>
        </div>
        {/* Statistik Box */}
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-4 mb-4">
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 flex flex-col items-center">
            <div className="text-2xl font-bold text-blue-700">{stats?.totalToday ?? 0}</div>
          ...
          <div className="bg-green-50 border border-green-200 rounded-lg p-4 flex flex-col items-center">
            <div className="text-2xl font-bold text-green-700">{stats?.totalMonth ?? 0}</div>
          ...
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 flex flex-col items-center">
            <div className="text-2xl font-bold text-yellow-700">{stats?.totalDraftMonth ?? 0}</div>
            <div className="text-sm text-yellow-800 mt-1 text-center">Pasien Draft Bulan Ini</div>
          </div>
          <div className="bg-red-50 border border-red-200 rounded-lg p-4 flex flex-col items-center">
            <div className="text-2xl font-bold text-red-700">{stats?.totalCancelledMonth ?? 0}</div>
            <div className="text-sm text-red-800 mt-1 text-center">Pasien Batal Bulan Ini</div>
          </div>
          <div className="bg-green-100 border border-green-400 rounded-lg p-4 flex flex-col items-center">
            <div className="text-2xl font-bold text-green-900">{stats?.totalCompletedMonth ?? 0}</div>
            <div className="text-sm text-green-900 mt-1 text-center">Pasien Terlayani Bulan Ini</div>
          </div>
        </div>
        <Card>
          <CardContent className="pt-6">
            <form onSubmit={(e) => { e.preventDefault(); router.get(route('pelayanan.rawat-jalan.index'), { search: localFilters.search, status: localFilters.status, tanggal: localFilters.tanggal }, { preserveState: true }); }} className="flex flex-col md:flex-row md:items-end gap-4">
              ...
              <div className="flex gap-4">
                <div>
                  <Label className="text-sm">Status Kunjungan</Label>
                ...
                <div>
                  <Label className="text-sm">Tanggal Kunjungan</Label>
                  <Input
                    type="date"
          ...
          <CardContent>
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full text-sm">
                ...
                <tbody>
                  {outpatientQueues.data.map((row, idx) => {
                    const statusObj = STATUS.find((s) => s.value === row.status);
                    return (
                      <tr key={row.id} className={`transition-colors duration-200 ${statusObj?.color || ''}`}>
                        <td className="px-3 py-2">{(outpatientQueues.current_page - 1) * outpatientQueues.per_page + idx + 1}</td>
                        ...
                        <td className="px-3 py-2">
                          <div className="flex gap-2">
                            {row.status === 'waiting' && (
                              <>
                                <Button size="icon" variant="ghost" title="Mulai Konsultasi" onClick={() => router.visit(route('konsultasi.show', row.id))}>
                                  <Stethoscope className="w-5 h-5" />
                                </Button>
                                <Button size="icon" variant="ghost" title="Batalkan" onClick={() => handleStatusUpdate(row.id, 'cancelled')}>
                            ...
                            {row.status === 'in_progress' && (
                              <>
                                <Button size="icon" variant="ghost" title="Lanjutkan Konsultasi" onClick={() => router.visit(route('konsultasi.show', row.id))}>
                                  <Stethoscope className="w-5 h-5" />
                                </Button>
                                <Button size="icon" variant="ghost" title="Batalkan" onClick={() => handleStatusUpdate(row.id, 'cancelled')}>
                                  <XCircle className="w-5 h-5 text-red-500" />
                            ...
                            {row.status === 'completed' && (
                              <Button size="icon" variant="ghost" title="Print Surat/Resume Medis" onClick={() => router.visit(route('resume-medis.print', row.id))}>
            ...
            {/* Pagination */}
            {outpatientQueues.last_page > 1 && (
              <div className="flex items-center justify-between mt-6">
                <p className="text-sm text-gray-500">
                  Menampilkan {((outpatientQueues.current_page - 1) * outpatientQueues.per_page) + 1} - {Math.min(outpatientQueues.current_page * outpatientQueues.per_page, outpatientQueues.total)} dari {outpatientQueues.total} data
                </p>
                <div className="flex gap-2">
                  {outpatientQueues.links.map((link: any, index: number) => (
                    <button
                      key={index}
                      onClick={() => {
                        if (link.url) {
